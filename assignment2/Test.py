...
def testInput(filename, expected):
    ...
    print("\nHamiltonian circuits expected: " +
          str(expected) + ", found: " + str(found))
    return found
...
def isValidHamiltonian(graph, circuit):
    ...
    for i in range(0, len(path)-1):
        vertex = path[i]
        if visited[vertex]:
            print("Error: Vertex " + str(vertex) +
                  " occurs at least twice in the circuit.")
            return False
        visited[vertex] = True
        if graph.getEdgeCount(vertex, path[i+1]) == 0:
            print("Error: The original graph has no edge between " +
                  str(vertex) + " and " + str(path[i+1]))
            return False
